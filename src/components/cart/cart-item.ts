...
export default class CartItemElem extends BaseComponent {
  ...
  constructor(product: CartItem, totalPrice: BaseComponent) {
    ...
    const printPrices = (diff = 0) => {
      ...
      this.inStock.getNode().textContent = `Осталось на складе: ${keyboardSwitch.quantity - this.currentNum}`;
      ...
      (this.cartInc.getNode() as HTMLButtonElement).disabled = this.currentNum === keyboardSwitch.quantity;
    };
    this.images = new ProductImage(keyboard.images);
    this.wrapper = new BaseComponent({ className: 'cart__container' });
    this.title = new BaseComponent({ tag: 'h2', className: 'cart__title', text: keyboard.title, parent: this.wrapper.getNode() });
    this.category = new BaseComponent({ className: 'cart__category', text: 'Клавиатура', parent: this.wrapper.getNode() });
    this.price = new BaseComponent({ className: 'cart__price', text: `${keyboardSwitch.price}₽`, parent: this.wrapper.getNode() });
    this.stockWrapper = new BaseComponent({ className: 'cart__stock-wrapper', parent:this.wrapper.getNode() });
    this.inStock = new BaseComponent({ className: 'cart__stock', text: keyboardSwitch.quantity > 0 ? `Осталось на складе: ${keyboardSwitch.quantity -1}` : 'Нет в наличии', parent: this.stockWrapper.getNode() });
    this.countBtn = new BaseComponent({ className: 'count-btn', parent: this.stockWrapper.getNode() });
    this.currentNum = quantity;
    this.cartDec = new Button({
      className: 'count-btn__dec',
      text: '-',
      parent: this.countBtn.getNode(),
      onclick: () => {printPrices(-1)},
    });
    ...
    this.cartDelete = new Button({ className: 'cart__stock cart__delete', text: 'Удалить', parent: this.stockWrapper.getNode(), onclick: () => {
