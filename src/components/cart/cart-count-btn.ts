...
export default class CartCountBtn extends BaseComponent {
  ...
  constructor(product: CartItem, inStock: BaseComponent) {
    ...
    const printQuantity = () => {
      ...
      inStock.getNode().textContent = `Осталось на складе ${keyboardSwitch.quantity - this.currentNum}`;
    }
    this.currentNum = quantity;
    this.cartDec = new Button({ className: 'count-btn__dec', text: '-' });
    this.count = new BaseComponent({ tag: 'span', className: 'count-btn__count', text: `${quantity}` });
    this.cartInc = new Button({ className: 'count-btn__inc', text: '+', onclick: () => {
      this.currentNum++;
