...
const brandsArr = ['Ducky', 'Leopold', 'Geekboards', 'Vortex', 'Mistel', 'Durgod', 'Tex', 'Shurikey Gear', 'Keychron', 'Varmilo', 'NuPhy', 'Topre Realforce', 'HHKB', 'Kinesis'];
...
export default class BrandFilter extends Filter {
  ...
  constructor(private productsState: ProductsListState) {
    ...
    this.brands = brandsArr.map((item) => new Button({ className: 'filter__btn', text: `${item}`, parent: this.filterWrapper.getNode() }));
    this.brands.map((item) => item.getNode().addEventListener('click', () => {
      // TODO REFACTOR
